...
var instantiateChaincode = async function(peers, channelName, chaincodeName, chaincodeVersion, chaincodeType, functionName, args, username, org_name, endorsePolicy) {
	...
	try {
		...
		var request = {
			...
			args: args,
			txId: tx_id,
      'endorsement-policy' : endorsePolicy
		  ...
		  //       }
			// 'endorsement-policy' : {
			//         identities: [
			// 										{ role: { name: 'member', mspId: 'IndMSP' }},
			// 										{ role: { name: 'member', mspId: 'KtbMSP' }},
			// 										{ role: { name: 'member', mspId: 'CmlMSP' }},
			// 										{ role: { name: 'member', mspId: 'PolMSP' }}
			// 									],
			//         policy: {
		...
		let results = null;
		if('1.0' == chaincodeVersion || 'v1' == chaincodeVersion || '1' == chaincodeVersion){
			console.log("instantiate");
			results = await channel.sendInstantiateProposal(request, 120000); //instantiate takes much longer
		} else {
			console.log("upgrade");
			results = await channel.sendUpgradeProposal(request, 120000); //instantiate takes much longer
		}
